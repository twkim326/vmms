...
%><%
/**
 * /sales/SalesReportMonthlyExcel.jsp
 *
 * 자판기 매출정보 > 조건별 매출집계 > 월별 > 엑셀
 *
 * 작성일 - 2011/04/05, 정원광
 *
 */
...
// 엑셀 생성
	...
	try {
		...
		sheet = workbook.createSheet("월별", 0);

		sheet.addCell(new Label(0, row, "페이지명", format1));
		sheet.addCell(new Label(1, row, "월별 매출집계", format5));
		sheet.mergeCells(1, row, 12, row);
		row++;
		...
		sheet.addCell(new Label(col, row, "날짜", format1));
		sheet.mergeCells(col, row, col, row + 1);
		sheet.setColumnView(col++, 10);
		sheet.addCell(new Label(col, row, objSales.data.get("ORGAN") + " 코드", format1)); //2017.12.21 jwhwang 추가
		sheet.mergeCells(col, row, col, row + 1);
		sheet.setColumnView(col++, 10);
		...
		//'자판기 매출정보 > 매출집계 > 월별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 Start
		...
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		sheet.setColumnView(col++, 15);
		//'자판기 매출정보 > 매출집계 > 월별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 End
		...
		sheet.addCell(new Label(col, row, "정보수집", format1));
		sheet.mergeCells(col, row, col + 1, row);
		sheet.addCell(new Label(col, row + 1, "건수", format1));
		sheet.setColumnView(col++, 10);
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		...
		for (int i = 0; i < objSales.list.size(); i++, row++) {
			...
			col = 0;
			sheet.addCell(new Label(col++, row, c.get("DATE"), format2));
...
//			sheet.addCell(new Number(col++, row, c.getLong("CNT_CARD"), format3));
			sheet.addCell(new Number(col++, row, c.getLong("CNT_PAYCO"), format3));		//'자판기 매출정보 > 매출집계 > 월별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
			sheet.addCell(new Number(col++, row, c.getLong("AMOUNT_PAYCO"), format3));	//'자판기 매출정보 > 매출집계 > 월별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
			...
			sheet.addCell(new Number(col++, row, c.getLong("CNT_ITEM"), format3));
			sheet.addCell(new Number(col++, row, c.getLong("AMOUNT_ITEM"), format3));
		...
		Formula formula = new Formula(col++, row, "SUM(F6:F" + row + ")", format4);
		...
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));		//'자판기 매출정보 > 매출집계 > 월별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
		sheet.addCell(formula.copyTo(col++, row));		//'자판기 매출정보 > 매출집계 > 월별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col, row));
...
// 파일명
	String excelName = StringEx.encode("월별매출집계");
