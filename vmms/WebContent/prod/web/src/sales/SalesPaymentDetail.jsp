...
<input type="hidden" id="sType" name="sType" value="03" />
...
<table id="detail" cellpadding="0" cellspacing="0" class="tableType06">
	...
	%>
		...
		<tr>
			...
			%>
		<tr onclick="_detail(<%=c.getLong("VM_PLACE_SEQ")%>, <%=c.getLong("AMOUNT_TOTAL")%>);" style="cursor:pointer;<%=payAmountTotal < 0L ? "background-color:#e9aaaa;" : ""%>">
			<%	if (StringEx.isEmpty(c.get("PARENT_ORGAN"))) { %>
			<td colspan="2" rowspan="<%=payCount%>" nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("ORGAN"), "-"))%></td>
			<%	} else { %>
...
<% if (false) { %>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_CNT_CASH"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("COMMISSION_CASH"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_AMOUNT_CASH"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_CNT_CARD"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("COMMISSION_CARD"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_AMOUNT_CARD"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_CNT_PAYCO"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("COMMISSION_PAYCO"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_AMOUNT_PAYCO"))%></td>
			<%	for (int j = 0; j < prepayCompanyCount; j++) { GeneralConfig s = (GeneralConfig) objSales.company.get(j); %>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_CNT_PREPAY_" + s.get("CODE")))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("COMMISSION_PREPAY_" + s.get("CODE")))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_AMOUNT_PREPAY_" + s.get("CODE")))%></td>
			<%	} %>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_CNT_TOTAL"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("COMMISSION_TOTAL"))%></td>
			<td rowspan="<%=payCount%>" class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_AMOUNT_TOTAL"))%></td>
<% } %>
			<%	for (int n = 1; n <= payCount; n++) {
					if (n > 1) { %>
		<tr onclick="_detail(<%=c.getLong("VM_PLACE_SEQ")%>, <%=c.getLong("AMOUNT_TOTAL")%>);" style="cursor:pointer;<%=payAmountTotal < 0L ? "background-color:#e9aaaa;" : ""%>">
			<%	} %>
			<td class="center number" nowrap><%=c.get("START_DATE_" + n)%> ~ <%=c.get("END_DATE_" + n)%></td>
			...
			<td class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_CNT_PAYCO_" + n))%></td>
			<td class="right number" nowrap><%=StringEx.comma(c.getLong("COMMISSION_PAYCO_" + n))%></td>
			<td class="right number" nowrap><%=StringEx.comma(c.getLong("PAY_AMOUNT_PAYCO_" + n))%></td>
				<%	for (int j = 0; j < prepayCompanyCount; j++) { GeneralConfig s = (GeneralConfig) objSales.company.get(j); %>
...
<script type="text/javascript">
	...
	<% } else { %>
		_submit($("form"), "SalesPaymentExcel.jsp", "_top");
	<% } %>
	}
