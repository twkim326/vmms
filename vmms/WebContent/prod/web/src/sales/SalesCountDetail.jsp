...
// 메서드 호출
	...
	try {
		if (pType.equals("0") ) // 당일 - tb_salescount 테이블에서 조회
			error = objSales.salesCountList(company, organ, depth, place, sDate, eDate, oMode, oType);
		else //과거 - tb_salescount_detail 테이블에서 조회 
			error = objSales.salesCountListNew(company, organ, depth, place, sDate, eDate, oMode, oType);
	} catch (Exception e) {
		error = e.getMessage();
	}
	...
	%>	
		...
		<tr>
			...
			<% 		}  
				...
				nPlace++;
				%>
		 <tr class="<%=c.getLong("TOTAL_PRICE")!=0 ?"bgAlertLine":""%>"  onclick="_detail(<%=c.getLong("ORGANIZATION_SEQ")%>, <%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.get("START_DATE")%>', '<%=c.get("END_DATE")%>', '<%=c.get("TERMINAL_ID")%>');" style="cursor:pointer;"> 
