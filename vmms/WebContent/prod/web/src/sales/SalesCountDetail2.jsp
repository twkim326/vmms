...
// 메서드 호출
	...
	try {
		if (pType.equals("0") ) // 당일 - tb_salescount 테이블에서 조회 --VM 1243
			error = objSales.salesCountList(company, organ, place, sDate, eDate, oMode, oType,sField,sQuery);
		else //과거 - tb_salescount_detail 테이블에서 조회 
			error = objSales.salesCountListNew(company, organ, place, sDate, eDate, oMode, oType,sField,sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<table id="detail" cellpadding="0" cellspacing="0" class="tableType06">
	<thead>
		<tr>
			...
			<th rowspan="2" nowrap><a href="javascript:_sort(3,<%=(oMode ==3 && oType == 0 ? 1 : 0)%>);" class="<%=(oMode == 3 ? (oType == 1 ? "desc" : "asc") : "none")%>" onmouseover="this.className = '<%=(oMode == 3 ? (oType == 1 ? "asc" : "desc") : "asc")%>';" onmouseout="this.className = '<%=(oMode == 3 ? (oType == 1 ? "desc" : "asc") : "none")%>';" title="판매기간별 정렬 <%=(oMode == 3 && oType == 0 ? "(↑)" : "(↓)")%>">마감기간</a></th>
...
<%	if (objSales.list.size() <= 0) { %>	
		<tr>
			<td class="center" colspan="<%=(prepayCompanyCount * 2 + 13)%>">해당 기간동안 발생한 마감이 없습니다</td>
		</tr>
	<%
	} else { //여기부터 scheo
	...
	%>	
		...
		<%
			...
			summaryColumnList.add("CNT_SMTPAY");	//scheo 
			summaryColumnList.add("AMOUNT_SMTPAY");
		...
		<%	
			...
			for (int i = 0; i < objSales.list.size(); i++) {
				...
				if (!StringEx.isEmpty(groupValues[0]) && !c.get(groupColumns[0]).equals(groupValues[0])) {
					...
					if (!StringEx.isEmpty(groupValues[2]) && nPlace > 1) {
		%>
		<tr>
			<td class="center number" colspan="2" nowrap>&nbsp;</td>
		 	<td class="center" colspan="4" style="background-color:#e9e9e9;" nowrap>소계(<%=groupValues[2]%>)</td>
			 		<%	for (int j = 0; j < summaryColumns.length; j++) { %>
			<td class="right number" style="background-color:#e9e9e9;" nowrap><%=StringEx.comma(summaryValues[2][j])%></td>
					<%	} %>
		...
		<tr>
			...
			<%	} else if ((groupValues[2] != null) && !c.get(groupColumns[2]).equals(groupValues[2])) { 
					if (!StringEx.isEmpty(groupValues[2]) && nPlace > 1) { %>
		<tr>
			<td class="center number" colspan="2" nowrap></td>
		 	<td class="center" colspan="4" style="background-color:#e9e9e9;" nowrap>소계(<%=groupValues[2]%>)</td>
			...
			<% 		}  
					...
					if (groupValues[2] == null) groupValues[2] = StringEx.setDefaultValue(c.get(groupColumns[2]), "");
		...
		<%if(c.getLong("TOTAL_PRICE")!=0){ %>
				 <tr class="bgAlertLine" onclick="_detail(<%=c.getLong("ORGANIZATION_SEQ")%>, <%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.get("START_DATE")%>', '<%=c.get("END_DATE")%>', '<%=c.get("TERMINAL_ID")%>');" style="cursor:pointer;"> 
		<!--<tr onclick="_detail(<%=c.getLong("ORGANIZATION_SEQ")%>, <%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.get("START_DATE")%>', '<%=c.get("END_DATE")%>', '<%=c.get("TERMINAL_ID")%>');" style="cursor:pointer;"> -->
			...
			<td class="center " nowrap><input type="button" class="button2" value="컬럼보기" onclick="_detail(<%=c.getLong("ORGANIZATION_SEQ")%>, <%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.get("START_DATE")%>', '<%=c.get("END_DATE")%>', '<%=c.get("TERMINAL_ID")%>');"  /></td>
		</tr>
		
		<%}else{ %>
		 <tr class="" onclick="_detail(<%=c.getLong("ORGANIZATION_SEQ")%>, <%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.get("START_DATE")%>', '<%=c.get("END_DATE")%>', '<%=c.get("TERMINAL_ID")%>');" style="cursor:pointer;"> 
		<!--<tr onclick="_detail(<%=c.getLong("ORGANIZATION_SEQ")%>, <%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.get("START_DATE")%>', '<%=c.get("END_DATE")%>', '<%=c.get("TERMINAL_ID")%>');" style="cursor:pointer;"> -->
			<%	if (StringEx.isEmpty(c.get("PARENT_ORGAN"))) { %>
			<td colspan="2" class="center" nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("ORGAN"), "-"))%></td>
			<%	} else { %>
		...
		<tr>
			...
			<td class="center number" colspan="2" nowrap>&nbsp;</td>
		 	<td class="center" colspan="4" style="background-color:#e9e9e9;" nowrap>소계(<%=groupValues[2]%>)</td>
			<%	} else { %>
		 	<td class="center" colspan="6" style="background-color:#e9e9e9;" nowrap>소계(<%=groupValues[2]%>)</td>
		 	<%	}
