...
%><%
/**
 * /sales/SalesReportGoodsDetail.jsp
 *
 * 자판기 매출정보 > 조건별 매출 현황 > 상품별
 *
 * 작성일 - 2011/04/05, 정원광
 *
 */
...
// 메서드 호출
	...
	try {
		error = objSales.product(sType, company, organ, depth, place, sDate, eDate, product, payTypes, oMode, oType);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<div class="infoBar">
	...
	<input type="button" class="button2" onclick="_summary_excel();" value="소계저장" style="right:80px;" />
...
<input type="hidden" name="_product" value="<%=product%>" />
...
<table cellpadding="0" cellspacing="0" cellspacing="0" class="tableType01">
	<thead>
		<tr>
			<th rowspan="2" nowrap><a href="javascript:_sort(0,<%=(oMode == 3 && oType == 0 ? 1 : 0)%>);" class="<%=(oMode == 3 ? (oType == 1 ? "desc" : "asc") : "none")%>" onmouseover="this.className = '<%=(oMode == 3 ? (oType == 1 ? "asc" : "desc") : "asc")%>';" onmouseout="this.className = '<%=(oMode == 3 ? (oType == 1 ? "desc" : "asc") : "none")%>';" title="상품별 정렬 <%=(oMode == 3 && oType == 0 ? "(↑)" : "(↓)")%>">상품</a></th>
			...
			<th rowspan="2" nowrap><a href="javascript:_sort(2,<%=(oMode == 2 && oType == 0 ? 1 : 0)%>);" class="<%=(oMode == 2 ? (oType == 1 ? "desc" : "asc") : "none")%>" onmouseover="this.className = '<%=(oMode == 2 ? (oType == 1 ? "asc" : "desc") : "asc")%>';" onmouseout="this.className = '<%=(oMode == 2 ? (oType == 1 ? "desc" : "asc") : "none")%>';" title="설치위치별 정렬 <%=(oMode == 2 && oType == 0 ? "(↑)" : "(↓)")%>">설치위치</a></th>
			<!-- //20120605 자판기코드 추가-->
	...
	<%
		...
		<tr>
		 	<td class="center" colspan="4" style="background-color:#e9e9e9;" nowrap>총계</td>
	...
	<%	}
		...
		// 2016011 중간에 소계 추가
		String[] groupColumns
			= (oMode == 3) ? new String[] { "ORGAN", "PRODUCT" }
			: (oMode == 2) ? new String[] { "ORGAN", "PLACE" }
			: new String[] { "PRODUCT", "ORGAN" };
		...
		<tr>
			<td class="center number" nowrap></td>
		 	<td class="center" colspan="3" style="background-color:#e9e9e9;" nowrap>소계(<%=groupValues[1]%>)</td>
		...
		<tr>
		 	<td class="center" colspan="4" style="background-color:#e9e9e9;" nowrap>합계(<%=groupValues[0]%>)</td>
		 	<% for (int j = 0; j < summaryColumns.length; j++) { %>
		...
		<tr onclick="_detail(<%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.getString("PRODUCT_CODE")%>', <%=c.getLong("AMOUNT_TOTAL")%>);" style="cursor:pointer;<%=c.getLong("AMOUNT_TOTAL") < 0L ? "background-color:#e9aaaa;" : ""%>">
			<td class="left" nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("PRODUCT"), "-"))%></td>
	...
	<% if (objSales.list.size() == 0) { %>
		...
		<!-- //20120605 자판기코드 추가-->
		<!-- 
			<td class="center" colspan="11">해당 기간동안 발생한 매출이 없습니다</td>
		 -->
			<td class="center" colspan="12">해당 기간동안 발생한 매출이 없습니다</td>
		</tr>
	<% } else { %>
		...
		<!-- //20120605 자판기코드 추가-->
		<!-- 
			<td class="center" colspan="3" style="background-color:#e9e9e9;" nowrap>합계</td>
		 -->	
		 	<td class="center" colspan="4" style="background-color:#e9e9e9;" nowrap>총계</td>
			<td class="right number" style="background-color:#e9e9e9;" nowrap><%=StringEx.comma(objSales.data.getLong("CNT_CASH"))%></td>
			<td class="right number" style="background-color:#e9e9e9;" nowrap><%=StringEx.comma(objSales.data.getLong("AMOUNT_CASH"))%></td>
...
<script type="text/javascript">
	...
	function _detail(place, product, amount) {
		$("place").value = place;
	...
	<% if ((objSales.list != null) && (objSales.list.size() > 65000)) { %>
		window.alert("65,000 라인을 초과하는 데이터입니다.\n\n보다 상세한 검색 조건으로 검색 후 다운로드하세요.");
	<% } else { %>
		_submit($("form"), "SalesReportGoodsExcel.jsp", "_top");
	<% } %>
	}
</script>