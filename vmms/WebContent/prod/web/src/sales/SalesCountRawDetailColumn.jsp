...
// 메서드 호출
	...
	try {
		if (pType.equals("0") )
			error = objSales.salesCountRaw(company, organSeq, depth, placeSeq, startDate, endDate, terminalId);
		else 
			error = objSales.salesCountRawNew(company, organSeq, depth, placeSeq, startDate, endDate, terminalId);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<table id="detail" cellpadding="0" cellspacing="0" class="tableType06">
	<thead>
		<tr>
			...
			<th rowspan="2" nowrap>비고</th>	<!-- scheo 20200107 실시간 집계 반영 -->
		</tr>
		<tr>
			<th nowrap>품명</th>
			<th nowrap>코드</th>
...
<%	if (objSales.list.size() <= 0) { %>	
		<tr>
			<td class="center" colspan="17">해당 기간동안 발생한 매출이 없습니다</td>
		</tr>
	<%
	} else {
	...
	%>	
		<tr>
			...
			<td class="right number" style="background-color:#e9e9e9;" nowrap></td>	<!-- scheo 20200107 실시간 집계 반영 -->
		</tr>	
		<%	
			for (int i = 0; i < objSales.list.size(); i++) {
				GeneralConfig c = (GeneralConfig) objSales.list.get(i); %>
		<tr class="<%=c.getLong("PRODUCT_PRICE")!=0 && c.getLong("PRODUCT_PRICE") != c.getLong("PRICE") ?"bgAlertLine": (i % 2 == 1 ? "bgLine" : "")%>" >		
			...
			<td nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("MEMO"), "-"))%></td>	<!-- scheo 20200107 실시간 집계 반영 -->
		</tr>
		<%	} %>		
		<tr>
			<td class="center" colspan="11" style="background-color:#e9e9e9;">총계</td>
...
<% } %>
	...
	<% } else { %>
		_submit($("form"), "SalesCountRawDetailColumnExcel.jsp", "_top");
	<% } %>
	}
</script>