...
// 메서드 호출
	...
	try {
		if (pType.equals("0") ) // 당일(17시 30분 이전) - tb_salescount 테이블에서 조회
			error = objSales.salesCountRawList(company, organ, depth, place, sDate, eDate, oMode, oType);
		else //당일(17시 30분 이후) - tb_salescount_detail 테이블에서 조회
...
<%	if (objSales.list.size() <= 0) { %>	
		<tr>
			<td class="center" colspan="13">해당 기간동안 발생한 마감이 없습니다</td>
		</tr>
	<%
	} else {
...
<script type="text/javascript">
	...
	function _detail(organSeq, placeSeq, startDate, endDate, terminalId) {
		...
		_submit($("form"), "SalesCountRawDetailColumn.jsp", "_self")
	}
	...
	<% } else { %>
		_submit($("form"), "SalesCountRawExcel.jsp", "_top");
	<% } %>
	}
