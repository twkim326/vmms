...
%><%
/**
 * /sales/SalesReportVMExcel.jsp
 *
 * 자판기 매출정보 > 조건별 매출집계 > 자판기별 > 엑셀
 *
 * 작성일 - 2011/04/05, 정원광
 *
 */
...
// 메서드 호출
	...
	try {
		error = objSales.vm(sType, company, organ,depth, place, sDate, eDate, payTypes, oMode, oType);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
// 엑셀 생성
	...
	try {
		...
		sheet = workbook.createSheet("자판기별", 0);

		sheet.addCell(new Label(0, row, "페이지명", format1));
		sheet.addCell(new Label(1, row, "자판기별 매출집계", format5));
		...
		//'자판기 매출정보 > 매출집계 > 자판기별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 Start
		...
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		sheet.setColumnView(col++, 15);
		//'자판기 매출정보 > 매출집계 > 자판기별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 End
		...
		for (int i = 0; i < objSales.list.size(); i++, row++) {
			...
			sheet.addCell(new Number(col++, row, c.getLong("CNT_PAYCO"), format3));		//'자판기 매출정보 > 매출집계 > 자판기별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 
			sheet.addCell(new Number(col++, row, c.getLong("AMOUNT_PAYCO"), format3));	//'자판기 매출정보 > 매출집계 > 자판기별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 
		...
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));		//'자판기 매출정보 > 매출집계 > 자판기별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 
		sheet.addCell(formula.copyTo(col++, row));		//'자판기 매출정보 > 매출집계 > 자판기별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col, row));
...
// 파일명
	String excelName = StringEx.encode("자판기별매출집계");
