...
// 메서드 호출
	...
	try {
		error = objVM.getSalesCountList(company, organ, pageNo, sDate, eDate, sField, sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<% for (int i = 0; i < objVM.list.size(); i++) { GeneralConfig c = (GeneralConfig) objVM.list.get(i); 
		if (c.getLong("COL_COUNT") + 1 != c.getLong("CNT")) {
			...
			if (c.getLong("COMPANY_SEQ")== 264) {
				// 코레일 유통일 경우 
				if ( c.getLong("COL_COUNT") > 24 && c.getLong("COL_COUNT") == c.getLong("COL_NO_MAX") && c.getLong("COL_NO_MIN") == 0 ) isAlert = false;
				else isAlert = true;
			} else {
...
%>	
		<tr class="<%=isAlert?"bgAlertLine": (i % 2 == 1 ? "bgLine" : "")%>" >		
			...
			<% if(cfg.getLong("user.company") == 0 || cfg.getLong("user.company") == 264) { %>
			<td class="center number" <%=c.get("COMPANY_SEQ").equals("264")?(c.get("PLACE_CD").equals(c.get("PLACE_CODE"))?"":"style='background-color: #ff0000'" ):""%> nowrap><%=Html.getText(c.get("PLACE_CD"))%></td>
			<%} %>
			<td class="center number" <%=c.get("COMPANY_SEQ").equals("264")?(c.get("PLACE_CODE").equals(c.get("PLACE_CD"))?"":"style='background-color: #ff0000'" ):""%> nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("PLACE_CODE"), "&nbsp;"))%></td>
...
<% if (objVM.list.size() == 0) { %>
		<tr>
			<td colspan="16" align="center">등록된 자판기가 없습니다</td>
		</tr>
<% } %>
	</tbody>
</table>
