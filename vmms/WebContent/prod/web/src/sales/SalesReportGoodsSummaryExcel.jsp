...
// 엑셀 생성
	...
	try {
		...
		summaryColumnList.add("CNT_PAYCO");
		summaryColumnList.add("AMOUNT_PAYCO");
		summaryColumnList.add("CNT_PREPAY");
		summaryColumnList.add("AMOUNT_PREPAY");
		summaryColumnList.add("CNT_ITEM");
		summaryColumnList.add("AMOUNT_ITEM");
		...
		long[] summaryValues = new long[summaryColumns.length];
		String good = null;
		
		for (int i = 0; i < objSales.list.size(); i++ ) {
			GeneralConfig c = (GeneralConfig) objSales.list.get(i);

			if (good != null && !c.get("GOODS").equals(good)) {
				
				
				col = 0;
				sheet.addCell(new Label(col++, row, good, format5));
				for (int j = 0; j < summaryColumns.length; j++) {
					sheet.addCell(new Number(col++, row, summaryValues[j], format3));
					
					summaryValues[j] = c.getLong(summaryColumns[j]);;	
				}
				
				good =  StringEx.setDefaultValue(c.get("GOODS"), "");
				row++;
			} else {
				if (good == null) good =  StringEx.setDefaultValue(c.get("GOODS"), "");
		...
		Formula formula = new Formula(col++, row, "SUM(B6:B" + row + ")", format4);
