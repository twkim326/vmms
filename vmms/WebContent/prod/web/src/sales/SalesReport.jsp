...
<form name="search" id="search" method="post" action="SalesReportDailyDetail.jsp" target="detail">
...
<table cellspacing="0" class="tableType02">
	...
	<th><span>소속/조직</span></th>
		...
		<% if (cfg.getLong("user.company") > 0) { // 소속이 지정되어 있는 계정 // %>
				<select id="organ0" class="js-example-basic-single js-example-responsive" style="width: 20%" onchange="CompanyEx.organ($('company').value.evalJSON(), 1, this.options[selectedIndex].value);">
		...
		<% } else { // 시스템 관리자 // %>
				<select id="organ0" class="js-example-basic-single js-example-responsive" style="width: 20%" onchange="Company.organ($('company').value.evalJSON(), 1, this.options[selectedIndex].value, 'C', null, 'Y');">
	...
	<tr>
		...
		<td>
			<input type="checkbox" id="payment" name="paymentAll" value="Y" class="checkbox" onclick="_checkAll(this);" checked> 전체
	...
	<tr>
		<th><span>집계단위</span></th>
		<td>
			<input type="radio" name="measure" onclick="_setMeasure('day');" checked> 일별
			<input type="radio" id="monthly" name="measure" onclick="_setMeasure('month');"> 월별
			<input type="radio" name="measure" onclick="_setMeasure('vm');"> 자판기별
			<input type="radio" name="measure" onclick="_setMeasure('column');"> 자판기컬럼별
			<input type="radio" name="measure" onclick="_setMeasure('product');"> 상품별
			<input type="text" id="product" name="product" class="txtInput" style="width:183px;display:none;" />
		</td>
	</tr>
	<tr>
		<th><span>집계기간</span></th>
		<td>
			...
			<img src="<%=cfg.get("imgDir")%>/web/icon_calendar.gif" style="cursor:pointer;" alt="날짜 선택" onclick="(function () {
					...
					Calendar.open($('sDate'), (function () { return _date($('sDate')); }), Event.pointerX(e), Event.pointerY(e), $('sDate').value);
				})(event)" />
			~
			...
			<img src="<%=cfg.get("imgDir")%>/web/icon_calendar.gif" style="cursor:pointer;" alt="날짜 선택" onclick="(function () {
					var e = event ? event : window.event;
					Calendar.open($('eDate'), (function () { return _date($('eDate')); }), Event.pointerX(e), Event.pointerY(e), $('eDate').value);
				})(event)" />
...
<div style="position:relative;">
	<a style="position:absolute; right:0; top:5px; font-size:11px; color:#888;">* 오늘 매출 현황은 상세 매출 현황과 다를 수 있습니다.</a>
</div>

<iframe id="detail" name="detail" src="SalesReportDailyDetail.jsp" scrolling="no" frameBorder="0" class="mt23" style="width:100%; height:207px; border-style:none;"></iframe>
...
<script type="text/javascript">
	...
	function _setMeasure(m) {
		...
		if (m == "month") {
			...
			o.period1.value = "지난달";
			o.period2.value = "2개월";
			o.period2.onclick = function() { _setDate(1); };
			o.period3.value = "3개월";
			o.period3.onclick = function() { _setDate(2); };

			o.search.action = "SalesReportMonthlyDetail.jsp";

			return;
		} else if (o.period0.value != "오늘") {
			o.sDate.value = "";
			o.eDate.value = "";
			...
			o.period2.onclick = function() { _setDate(1); };
			o.period3.value = "3개월";
			o.period3.onclick = function() { _setDate(3); };
		}
		...
		if (m == "product") {
			...
			o.search.action = "SalesReportGoodsDetail.jsp";
		}  else if (m == "column") {
			o.search.action = "SalesReportVMColumnDetail.jsp";
		} else if (m == "vm") {
			o.search.action = "SalesReportVMDetail.jsp";
		} else {
			o.search.action = "SalesReportDailyDetail.jsp";
		}
	}
	...
	function _setDate(n) {
		...
		if ($("monthly").checked) {
			if (n > 0) {
				var s = new Date(d.getFullYear(), d.getMonth() - n, 1);

				sDate.value = s.getFullYear() + (s.getMonth() < 9 ? "0" : "") + (s.getMonth() + 1);
				eDate.value = d.getFullYear() + (d.getMonth() < 9 ? "0" : "") + (d.getMonth() + 1);
			} else {
				var s = new Date(d.getFullYear(), d.getMonth() + n, 1);

				sDate.value = s.getFullYear() + (s.getMonth() < 9 ? "0" : "") + (s.getMonth() + 1);
				eDate.value = s.getFullYear() + (s.getMonth() < 9 ? "0" : "") + (s.getMonth() + 1);
			}
		} else {
			if (n > 0) {
	...
	function _search() {
		...
		if (!o.sDate.value) {
			window.alert("[집계기간 - 시작일]을 입력하세요.");
			return;
		}
		if (!o.eDate.value) {
			window.alert("[집계기간 - 종료일]를 입력하세요.");
			return;
		}
		if (o.sDate.value > o.eDate.value) {
			window.alert("[집계기간 - 종료일]이 [집계기간 - 시작일]보다 이전입니다.");
			return;
		}
