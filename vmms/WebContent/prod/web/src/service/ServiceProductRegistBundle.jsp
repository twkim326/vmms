	...
	if (request.getMethod().equals("POST")) {
		...
		if ( company == 0) {
			out.print("<script language='javascript'>"
					+ "window.alert('필수 입력값이 존재하지 않습니다.');"
					+ "top.location.reload();"
					+ "</script>");
		...
		try {
			error = objProduct.regist(company, excel);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<div id="window">
	...
	<table cellspacing="0" class="tableType03 tableType05" id="regist">
		...
		<tr>
			<th><span>소속 *</span></th>
			<td>
				<select name="company"  id="company" class="checkForm<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%>" <%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%> option='{"isMust" : true, "message" : "소속을 선택하세요."}'>
	...
	<div style="border:1px solid #ddd; padding:10px; color:#888; font-size:11px; margin-top:15px; position:relative;">
		* 반드시 정해진 형식에 맞는 엑셀을 업로드하세요.
		<a href="<%=cfg.get("topDir")%>/common/src/down.jsp?src=product.xls" style="color:#888; font-weight:bold; font-size:11px; position:absolute; right:10px; top:10px;">샘플보기</a>
	</div>
</div>
