...
%><%
/**
 * /service/ServiceProductCategory.jsp
 *
 * 서비스 > 상품 > 그룹
 *
 * 작성일 - 2017/05/19, 정원광
...
// 메서드 호출
	...
	try {
		error = objVM.getPlaceTerminalList(company, organ);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
// 목록 생성
	...
	for (int i = 0; i < objVM.terminal.size(); i++) {
		GeneralConfig c = (GeneralConfig) objVM.terminal.get(i);

		responseText += (StringEx.isEmpty(responseText) ? "" : ",")
				+ "{"
					+ "\"seq\" : " + c.getLong("SEQ")
					+ ", \"terminal_id\" : \"" + c.get("TERMINAL_ID").trim() + "\""
					+ ", \"place\" : \"" + c.get("PLACE").trim() + "\""
				+ "}"
			;
	}
