...
%><%
/**
 * /service/ServiceProductModify.jsp
 *
 * 서비스 > 상품 > 수정
 *
 * 작성일 - 2017/05/22, 황재원
...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		if (category == 0 || StringEx.isEmpty(code) || StringEx.isEmpty(name)) {
			out.print("{\"code\" : \"FAIL\", \"message\" : \"" + URLEncoder.encode("필수 입력값이 존재하지 않습니다.", "UTF-8") + "\"}");
			return;
		}

		try {
			error = objProduct.regist(seq, company, vendor, category, code, name, price, barcode, memo, false);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<div id="window">
	...
	<table cellspacing="0" class="tableType03 tableType05" id="regist">
		...
		<tr>
			<th><span>소속 *</span></th>
			<td>
				<select id="company" class="checkForm<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%> js-example-basic-single js-example-responsive" style="width: 40%" onchange="_category(this.options[selectedIndex].value, 0);"<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%>>
					<option value="0">- 선택하세요</option>
			<% for (int i = 0; i < objProduct.company.size(); i++) { GeneralConfig c = (GeneralConfig) objProduct.company.get(i); %>
					<option value="<%=c.getLong("SEQ")%>"<%=(c.getLong("SEQ") == objProduct.data.getLong("COMPANY_SEQ") ? " selected" : "")%>><%=c.get("NAME")%></option>
			<% } %>
				</select>
			</td>
		</tr>
		<tr>
			...
			<% for (int i = 0; i < objProduct.vendor.size(); i++) { GeneralConfig c = (GeneralConfig) objProduct.vendor.get(i); %>
					<option value="<%=c.getLong("SEQ")%>"<%=(c.getLong("SEQ") == objProduct.data.getLong("VENDOR_SEQ") ? " selected" : "")%>><%=c.get("NAME")%></option>
			<% } %>					
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td><input type="text" id="code" class="checkForm txtInput disabled" value="<%=Html.getText(objProduct.data.get("CODE"))%>"  maxlength="20" option='{"isMust" : true, "varType" : "ALNUM", "message" : "코드를 입력하세요."}' /> </td>
		</tr>
		<tr>
		...
		<tr>
			<th><span>상품명 *</span></th>
			<td><input type="text" id="name" class="checkForm txtInput" value="<%=Html.getText(objProduct.data.get("NAME"))%>" maxlength="100" style="width:370px" option='{"isMust" : true, "message" : "상품명을 입력하세요."}' /></td>
		</tr>
		<tr>
			<th><span>단가</span></th>
			<td><input type="text" id="price" class="checkForm txtInput" value="<%=Html.getText(objProduct.data.get("PRICE"))%>" maxlength="20" /></td>
		</tr>
		<tr>
			<th><span>설명</span></th>
			<td><input type="text" id="memo" class="checkForm txtInput" value="<%=Html.getText(objProduct.data.get("MEMO"))%>" maxlength="200" style="width:370px" /></td>
		</tr>
		<tr>
			<th class="last"><span>바코드</span></th>
			<td class="last"><input type="text" id="barcode" value="<%=Html.getText(objProduct.data.get("BAR_CODE"))%>" class="checkForm txtInput" maxlength="20" /></td>
		</tr>
	</table>
...
<%
	if (objProduct.data.getLong("CATEGORY_SEQ") > 0) {
		if (objProduct.data.getInt("CATEGORY_DEPTH") == 0) {
			out.println("_category(" + objProduct.data.getLong("COMPANY_SEQ") + ", 0, " + objProduct.data.getLong("CATEGORY_SEQ") + ")");
			out.println("_category(" + objProduct.data.getLong("COMPANY_SEQ") + ", " + objProduct.data.getLong("CATEGORY_SEQ") + ", " + objProduct.data.getLong("CATEGORY_SEQ") + ")");
		} else {
			out.println("_category(" + objProduct.data.getLong("COMPANY_SEQ") + ", 0, " + objProduct.data.getLong("PARENT_SEQ") + ")");
			out.println("_category(" + objProduct.data.getLong("COMPANY_SEQ") + ", " + objProduct.data.getLong("PARENT_SEQ") + ", " + objProduct.data.getLong("CATEGORY_SEQ") + ")");
		}
	} else {
		out.println("_category(" + objProduct.data.getLong("COMPANY_SEQ") + ", 0)");
	}
%>
//	}.bind(this));
</script>