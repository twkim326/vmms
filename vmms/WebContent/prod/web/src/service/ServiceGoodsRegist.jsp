...
%><%
/**
 * /service/ServiceGoodsRegist.jsp
 *
 * 서비스 > 상품 > 등록
 *
 * 작성일 - 2011/04/02, 정원광
...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		if (group == 0 || (!isAuto && StringEx.isEmpty(code)) || StringEx.isEmpty(name)) {
			out.print("{\"code\" : \"FAIL\", \"message\" : \"" + URLEncoder.encode("필수 입력값이 존재하지 않습니다.", "UTF-8") + "\"}");
			return;
		}

		try {
			error = objGoods.regist(0, company, group, code, name, barcode, memo, isAuto);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<script type="text/javascript">
	function _group(company, group) {
		var o = {company : $('company'), maker : $('maker'), group : $('group')};
