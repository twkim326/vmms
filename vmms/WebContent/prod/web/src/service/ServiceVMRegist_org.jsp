	...
	if (request.getMethod().equals("POST")) {
		...
		*/
		...
		try {
			...
			error = objVM.regist(request, seq, company, organ, user, code, terminal, model, place, modem, sgcnt, strReflectFlag, aspCharge);
		} catch (Exception e) {
			error = e.getMessage();
		}
		...
		out.print(Message.refresh("ServiceVMRegist.jsp?seq=" + error + addParam, "설정하신 내용의 등록이 완료되었습니다.", "parent"));
		return;
	} else {
		try {
...
-->	
	<tr>
		...
		<td colspan="3">
			...
			<select name="company" id="company" class="checkForm<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%>" onchange="CompanyEX2.organ(this.options[selectedIndex].value.evalJSON(), 0, 0);CompanyEX2.organDup(this.options[selectedIndex].value.evalJSON(), 0, 0,'B');
				_group(this.options[selectedIndex].value.evalJSON().seq, 0);
	...
	<tr>
		...
		<td colspan="3">
			...
			<span id="organ">
				<select id="organ0" class="checkForm" onchange="CompanyEX2.organ($('company').value.evalJSON(), 1, this.options[selectedIndex].value);">
...
<% } %>
	...
	<tr>
		<th<% if (objVM.error.size() == 0) { %> class="last"<% } %>><span>설치 위치</span></th>
		<td<% if (objVM.error.size() == 0) { %> class="last"<% } %> colspan="3"><input type="text" name="place" id="place" value="<%=Html.getText(objVM.data.get("PLACE"))%>" class="checkForm txtInput" style="width:350px" maxlength="100" option='{"isMust" : true, "message" : "설치 위치를 입력하세요."}' /></td>
	</tr>
...
<table cellspacing="0" class="tableType04">
	...
	%>
		<td>
			<span>상품 검색</span>
			<select id="maker" onchange="_group(<%=(cfg.get("user.operator").equals("Y") ? cfg.getLong("user.company") : "$('company').value.evalJSON().seq")%>, this.options[selectedIndex].value);">
			...
			<input type="button" value="검색" class="button1" onclick="_goods();" />&nbsp;&nbsp;&nbsp;&nbsp;<span>기판매상품처리기준</span><select id="reflectflag" name="reflectflag" onchange="javascrit:changeleflect();">
	...
	<tr>
		<td style="border-top:1px solid #ccc;">
			...
			<div class="box" style="width:600px;">
				<input type="hidden" name="sgcnt" id="sgcnt" value="<%=objVM.goods.size()%>" />
				<table cellpadding="0" cellspacing="0" id="sArea" style="<%=(objVM.goods.size() == 0 ? "display:none;" : "")%>">
					<colgroup>
						<col width="50" />
						<col width="500"/>
						<col width="30" />
			...
			<% for (int i = 0; i < objVM.goods.size(); i++) { GeneralConfig c = (GeneralConfig) objVM.goods.get(i); %>
					<input type="hidden" name="seq<%=(i + 1)%>" id="seq<%=(i + 1)%>" value="<%=c.getLong("GOODS_SEQ")%>" />
					<tr>
						<td><input type="text" name="col<%=(i + 1)%>" id="col<%=(i + 1)%>" value="<%=c.getInt("COL_NO")%>" maxlength="2" class="input col" /></td>
			...
			<% } %>
				...
				<ul id="sNone" style="<%=(objVM.goods.size() == 0 ? "" : "display:none;")%>">
...
<style type="text/css">
	...
	div.box table td input.col {width:40px; padding:0 4px; text-align:right;}
	div.box table td input.name {width:480px;padding:0 4px; }
...
<script type="text/javascript">
	...
	function _group(company, group, n) {
		var o = {maker : $('maker'), group : $('group'), goods : $('goods'), sbmsg : $('sbmsg')};
		...
		o.goods.update('<li><a class="none">상품을 검색하세요</a></li>');
		o.sbmsg.show();
	...
	function _goods() {
		var o = {maker : $('maker'), group : $('group'), sField : $('sField'), sQuery : $('sQuery'), goods : $('goods'), sbmsg : $('sbmsg')};
		...
		new Ajax.Request('ServiceVMGoodsList.jsp', {
			asynchronous : true,
			parameters : {
				group : o.group.selectedIndex == 0 ? o.maker.options[o.maker.selectedIndex].value : o.group.options[o.group.selectedIndex].value
			...
			onSuccess : function (xmlHttp) {
				try {
					...
					e.data.each (function (data, i) {
						o.goods.insert('<li><a onclick="_move(' + data.seq + ', \'' + decodeURIComponent(data.name) + '\');" title="' + decodeURIComponent(data.name) + '">' + data.code + ' | ' + decodeURIComponent(data.name) + '</a></li>');
					}.bind(this));

					if (e.data.length == 0) {
						o.goods.update('<li><a class="none">등록된 상품이 없습니다</a></li>');
					}
				} catch (e) {
					window.alert('예기치 않은 오류가 발생하였습니다.');
				}
	...
	function _move(seq, name) {
		...
		$('sList').insert('<input type="hidden" name="seq' + sgcnt + '" id="seq' + sgcnt + '" value="' + seq + '" />'
			+ '<tr>'
			+ '<td><input type="text" name="col' + sgcnt + '" id="col' + sgcnt + '" maxlength="2" class="input col" /></td>'
...
//		}
		...
		if (!confirm('<%=(StringEx.isEmpty(confirm) ? "" : confirm + "\\n\\n")%>등록하시겠습니까?')) {
			return;
		}
...
<% if (cfg.get("user.operator").equals("Y")) { %>
		_group(<%=cfg.getLong("user.company")%>, 0);
		...
		<%}%>		
<% } else if (seq > 0) { %>
		_group(<%=objVM.data.getLong("COMPANY_SEQ")%>, 0);
		CompanyEX2.depth = <%=cfg.getInt("user.organ.depth")%>;
