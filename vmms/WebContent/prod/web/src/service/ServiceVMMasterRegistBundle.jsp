...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		try {
			error = objVM.master(company, excel);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<div id="window">
	...
	<div style="border:1px solid #ddd; padding:10px; color:#888; font-size:11px; margin-top:15px; position:relative;">
		* 반드시 정해진 형식에 맞는 엑셀을 업로드하세요.
		<a href="<%=cfg.get("topDir")%>/common/src/down.jsp?src=vm.xls" style="color:#888; font-weight:bold; font-size:11px; position:absolute; right:10px; top:10px;">샘플보기</a>
	</div>
</div>
