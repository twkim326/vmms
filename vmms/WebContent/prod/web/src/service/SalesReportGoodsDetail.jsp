...
<table cellpadding="0" cellspacing="0" cellspacing="0" class="tableType01">
	...
	<%	}
		...
		// 2016011 중간에 소계 추가
		String[] groupColumns
			= (oMode == 3) ? new String[] { "ORGAN", "GOODS" }
			: (oMode == 2) ? new String[] { "ORGAN", "PLACE" }
			: new String[] { "GOODS", "ORGAN" };
		String[] summaryColumns = {
				"CNT_CASH", "AMOUNT_CASH", "CNT_CARD", "AMOUNT_CARD", 
				"CNT_PREPAY", "AMOUNT_PREPAY", "CNT_TOTAL", "AMOUNT_TOTAL"
			};
		...
		<tr onclick="_detail(<%=c.getLong("VM_PLACE_SEQ")%>, '<%=c.getString("PRODUCT_CODE")%>', <%=c.getLong("AMOUNT_TOTAL")%>);" style="cursor:pointer;<%=c.getLong("AMOUNT_TOTAL") < 0L ? "background-color:#e9aaaa;" : ""%>">
			<td class="left" nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("GOODS"), "-"))%></td>
