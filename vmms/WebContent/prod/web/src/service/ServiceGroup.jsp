...
// 메서드 호출
	...
	try {
		error = objGroup.getList(company, cate, pageNo, sField, sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<div class="infoBar mt23">
	검색 결과 : <strong><%=StringEx.comma(objGroup.records)%></strong>건
<% if (cfg.isAuth("I")) { %>
...
<table cellspacing="0" class="tableType01">
	...
	<tbody>
<% for (int i = 0; i < objGroup.list.size(); i++) { GeneralConfig c = (GeneralConfig) objGroup.list.get(i); %>
...
<!-- 페이징 -->
<div class="paging"><%=Pager.getList(request, pageNo, cfg.getInt("limit.page"), objGroup.pages, "", "page", "", buttons)%></div>
<!-- # 페이징 -->
...
<script type="text/javascript">
	...
	function _bundle() {
		new IFrame(510, 225, 'ServiceGroupRegistBundle.jsp').open();
	}

	function _regist() {
		new IFrame(510, 245, 'ServiceGroupRegist.jsp?depth=1').open();
	}

	function _modify(n) {
		new IFrame(510, 245, 'ServiceGroupModify.jsp?seq=' + n).open();
	}
