...
// 메서드 호출
	...
	try {
		error = objVM.getList(company, organ, aspCharge, pageNo, sField, sQuery, collectChk);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<table cellspacing="0" class="tableType02">
	...
	<tr>
		...
		<td>
			<select id="sField" name="sField" >
				...
				<option value="C.ID"<%=sField.equals("C.ID") ? " selected" : ""%>>담당자 계정</option>
	...
	<tr>
		...
		<td>
			...
			<span id="organ" style="width: 20%">
				<select id="organ0" class="js-example-basic-single js-example-responsive" onchange="Company.organ($('company').value.evalJSON(), 1, this.options[selectedIndex].value);" style="width: 15%">
...
<div class="infoBar mt23">
	...
	<% if (cfg.isAuth("I")) { %>
		<input type="button" class="button2" value="개별등록" onclick="location.href = 'ServiceVMRegist.jsp?Regitype=0<%=addParam%>';" />
...
<table cellpadding="0" cellspacing="0" cellspacing="0" class="tableType01">
	...
	<thead>
		<tr>
			...
			<th nowrap>등록일</th>
	<% if ( cfg.isAuth("D")) { %>
			<th nowrap bgcolor="#f5f5dc">사업자번호</th>
			<th nowrap bgcolor="#f5f5dc">BIZTYPE</th>
			<th nowrap bgcolor="#f5f5dc"> 상호명</th>
	<%}%>
	<% if (cfg.isAuth("U") || cfg.isAuth("D")) { %>
			<th nowrap>관리</th>
	<% } %>
...
<% for (int i = 0; i < objVM.list.size(); i++) { GeneralConfig c = (GeneralConfig) objVM.list.get(i); %>
		...
		<% if ( cfg.isAuth("D")) { %>
			<td class="center number" style="background-color: beige" nowrap><%=Html.getText(c.get("BUSINESSNO"))%></td>
			<td class="center number" style="background-color: beige" nowrap><%=Html.getText(c.get("BIZTYPE"))%></td>
			<td class="center number" style="background-color: beige" nowrap><%=Html.getText(c.get("MERCHANTNAME"))%></td>
		<% } %>
		<% if (cfg.isAuth("U") || cfg.isAuth("D")) { %>
			<td class="center" nowrap>
				<% if (cfg.isAuth("U")) { %>
				<input type="button" class="button1" value="수정" onclick="location.href = 'ServiceVMRegist.jsp?UID=<%=c.get("USER_ID")%>&Regitype=1&seq=<%=c.getLong("SEQ") + addParam%>';" />
				<% } %>
				<% if (cfg.isAuth("D")) { %>
...
<% if (objVM.list.size() == 0) { %>
		<tr>
			...
			<%}else if (cfg.isAuth("U")) {%>
				<td colspan="12" align="center">등록된 자판기가 없습니다</td>
			<%}else {%>
				<td colspan="11" align="center">등록된 자판기가 없습니다</td>
			<%}%>
		</tr>
<% } %>
	</tbody>
