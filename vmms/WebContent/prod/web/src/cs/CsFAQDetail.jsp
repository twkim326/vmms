...
// 메서드 호출
	...
	try {
		error = objFAQ.detail(seq, cate, sField, sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<table cellspacing="0" class="tableType03">
	...
	<tr>
		<th><span>제목</span></th>
		<td><strong><%=(StringEx.isEmpty(objFAQ.data.get("CATEGORY")) ? "" : "[" + Html.getText(objFAQ.data.get("CATEGORY")) + "] ")%><%=Html.getText(objFAQ.data.get("TITLE"))%></strong></td>
	</tr>
	<tr>
		<th><span>조회</span></th>
		<td><%=objFAQ.data.getLong("READS")%></td>
	</tr>
	<tr>
		<th><span>날짜</span></th>
		<td><%=objFAQ.data.get("CREATE_DATE")%></td>
	</tr>
	<% if (seq==12) { %> 
	<tr>
		<th><span>내용</span></th>
		<td><%=objFAQ.data.get("DETAIL")%></td>
	</tr>
	...
	<tr>
		...
		<td class="last"><%=objFAQ.data.get("DETAIL")%></td>
	</tr>
	<%}%>
</table>

<div class="buttonArea" style="text-align:right; position:relative; margin-top:10px;">
	<input type="button" value="목록" class="button1" onclick="location.href = 'CsFAQ.jsp?1=1<%=addParam%>';" style="position:absolute; left:0; top:0; " />
	&nbsp;
<% if (cfg.isAuth("D")) { %>
	<input type="button" value="삭제" class="button1" onclick="_delete(<%=seq%>);" />
<% } %>
<% if (cfg.isAuth("U")) { %>
	<input type="button" value="수정" class="button1" onclick="location.href = 'CsFAQRegist.jsp?seq=<%=seq + addParam%>';" />
<% } %>
</div>

<% if (objFAQ.data.getLong("PREV.SEQ") > 0 || objFAQ.data.getLong("NEXT.SEQ") > 0) { %>
<table cellspacing="0" class="tableType03" style="margin-top:15px;">
	<colgroup>
		<col width="130" />
		<col width="*"/>
	</colgroup>
	<% if (objFAQ.data.getLong("NEXT.SEQ") > 0) { %>
	<tr onclick="location.href = '?seq=<%=objFAQ.data.getLong("NEXT.SEQ") + addParam%>';" style="cursor:pointer;">
		<th class="<%=(objFAQ.data.getLong("PREV.SEQ") > 0 ? "" : "last")%>"><span>다음글</span></th>
		<td class="<%=(objFAQ.data.getLong("PREV.SEQ") > 0 ? "" : "last")%>" colspan="3"><%=Html.getText(objFAQ.data.get("NEXT.TITLE"))%></td>
	</tr>
	<% } %>
	<% if (objFAQ.data.getLong("PREV.SEQ") > 0) { %>
	<tr onclick="location.href = '?seq=<%=objFAQ.data.getLong("PREV.SEQ") + addParam%>';" style="cursor:pointer;">
		<th class="last"><span>이전글</span></th>
		<td class="last" colspan="3"><%=Html.getText(objFAQ.data.get("PREV.TITLE"))%></td>
	</tr>
	<% } %>
</table>
<% } %>
...
<script type="text/javascript">
	function _delete(seq) {
		...
		new Ajax.Request('CsFAQDelete.jsp', {
			asynchronous : false,
			parameters : {seq : seq},
			onSuccess : function (xmlHttp) {
				try {
					...
					if (e.code == 'SUCCESS') {
						...
						location.replace('CsFAQ.jsp?1=1<%=addParam%>');
					} else {
