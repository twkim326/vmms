...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		try {
			error = objNotice.regist(seq, cate, title, detail);
		} catch (Exception e) {
			error = e.getMessage();
		}
		...
		out.print(Message.refresh("CsNoticeDetail.jsp?seq=" + error + addParam, "입력하신 내용의 등록이 완료되었습니다.", "parent"));
		return;
	} else {
		try {
			error = objNotice.regist(seq);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<% if (objNotice.cate.size() > 0) { %>
	...
	<% for (int i = 0; i < objNotice.cate.size(); i++) { GeneralConfig c = (GeneralConfig) objNotice.cate.get(i); %>
				<option value="<%=c.get("CODE")%>"<%=c.get("CODE").equals(objNotice.data.get("CATEGORY")) ? " selected" : ""%>><%=c.get("NAME")%></option>
	<% } %>
			</select>
		</td>
	</tr>
<% } %>
	<tr>
		<th><span>제목</span></th>
		<td><input type="text" name="title" id="title" value="<%=Html.getText(objNotice.data.get("TITLE"))%>" class="checkForm txtInput" style="width:592px" maxlength="100" option='{"isMust" : true, "message" : "제목을 입력하세요."}' /></td>
	</tr>
	<tr>
		<th class="last"><span>내용</span></th>
		<td class="last">
			<script type="text/javascript" src="<%=cfg.get("topDir")%>/common/module/ckeditor/ckeditor.js"></script>
			<textarea name="detail" id="detail"><%=Html.getText(objNotice.data.get("DETAIL"))%></textarea>
...
<div class="buttonArea">
	...
	<input type="button" value="" class="btnList" onclick="location.href = 'CsNotice.jsp?1=1<%=addParam%>';" />
</div>
</form>
