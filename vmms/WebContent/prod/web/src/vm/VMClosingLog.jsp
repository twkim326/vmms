...
// 설정
	GlobalConfig cfg = new GlobalConfig(request, response, session, "0305");
...
// 메서드 호출
	...
	try {
		error = objVM.getClosingList(company, organ, pageNo, sDate, eDate, sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<% for (int i = 0; i < objVM.list.size(); i++) { GeneralConfig c = (GeneralConfig) objVM.list.get(i); %>
		<tr class="<%=(i % 2 == 1 ? "bgLine" : "")%>" >		
			...
			<td class="center number"><%=Html.getText(c.get("MODEL"))%></td>
			<td class="center number"><%=Html.getText(c.get("TERMINAL_ID"))%></td>
			<td class="center" nowrap><%=Html.getText(c.get("CLOSING_DATE"))%></td>
			<td class="center" nowrap><%=Html.getText(c.get("REG_DATE"))%></td>
			<td class="center " <%=(c.getInt("SALES_COUNT")==0 ?"style='background-color: #ff0000'":"")%> nowrap><%=c.getInt("SALES_COUNT")==0?"":"&nbsp;<input type=\"button\" class=\"button1\" value=\"컬럼\" onclick=\"_detail('" + c.get("TERMINAL_ID")+ "', '" + c.get("CLOSING_DATE")+ "')\" />"%></td>
		</tr>		
<% } %>
<% if (objVM.list.size() == 0) { %>
		<tr>
			<td colspan="9" align="center">등록된 자판기가 없습니다</td>
		</tr>
<% } %>
	</tbody>
</table>
