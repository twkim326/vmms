...
// 설정
	GlobalConfig cfg = new GlobalConfig(request, response, session, "0306");
...
// 메서드 호출
	...
	try {
		error = objVM.getKTCCertKeyRenewalList(company, organ, pageNo, sDate, eDate, sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<% for (int i = 0; i < objVM.list.size(); i++) { GeneralConfig c = (GeneralConfig) objVM.list.get(i); %>
		<tr class="<%=(i % 2 == 1 ? "bgLine" : "")%>" >		
			...
			<td class="center number" nowrap><%=Html.getText(c.get("TRANSACTION_NO"))%></td>
			<td class="center number" nowrap><%=Html.getText(c.get("CODE"))%></td>
			<td class="center number" nowrap><%=Html.getText(c.get("TERMINAL_ID"))%></td>
			<td class="center" nowrap><%=Html.getText(c.get("RENEWAL_DATE"))%></td>
		</tr>		
<% } %>
