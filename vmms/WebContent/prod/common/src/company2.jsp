...
// 전송된 내용
	...
	long userorgan = StringEx.str2long(request.getParameter("userorgan"), 0);
...
//// 조직 트리 가져오기(담당자 조직트리 가져오기)
	String UserorganText = "";

	if (StringEx.isEmpty(error) && userorgan > 0) {
		try {
			//20160108 INDEX 힌트 추가
			//20160113 정렬순성에 SEQ 추가
			//ps = dbLib.prepareStatement(conn, "SELECT SEQ FROM TB_ORGANIZATION " + (isAll.equals("N") ? "WHERE IS_ENABLED = 'Y'" : "") + " START WITH SEQ = " + userorgan + " CONNECT BY SEQ = PRIOR PARENT_SEQ ORDER BY DEPTH");
			ps = dbLib.prepareStatement(conn,
					"SELECT /*+ INDEX(A) */"
							+ " SEQ"
						+ " FROM TB_ORGANIZATION A"
						+ (isAll.equals("N") ? " WHERE IS_ENABLED = 'Y'" : "")
						+ " START WITH SEQ = " + userorgan
						+ " CONNECT BY SEQ = PRIOR PARENT_SEQ"
						+ " ORDER BY DEPTH, SEQ"
				);
			rs = ps.executeQuery();
			...
			while (rs.next()) {
				UserorganText += (StringEx.isEmpty(UserorganText) ? "" : ",")
						+ "{"
							+ "\"seq\" : " + rs.getLong("SEQ")
						+ "}"
					;
...
// 결과 출력
	if (StringEx.isEmpty(error)) {
		out.print("{\"code\" : \"SUCCESS\", \"data\" : [" + responseText + "], \"organ\" : [" + organText + "], \"userorgan\" : [" + UserorganText + "]}");
	} else {
		out.print("{\"code\" : \"FAIL\"}");
	}
%>