...
public class Sales2 {
	 ...
	 * 생성자
	 */
	public Sales2(GlobalConfig cfg) throws Exception {
	// set config
		this.cfg = cfg;
...
//		}
	...
	// 목록 가져오기
		...
		try {
			long[] cnt = {0, 0, 0, 0, 0};	//scheo 2018.10.10 추가
			long[] amt = {0, 0, 0, 0, 0};	//scheo 2018.10.10 추가
...
// 정렬 값(oMode, oType)에 따라 ORDER BY 변경
	+ ((oType == 1) ? (	// DESC
			...
			this.data.put("AMOUNT_CASH", amt[2]);
	 ...
	 *
	 */
	public String Detail_Basket(String TERMINAL_ID, String TRANSACTION_NO,long NO,
								String TRANSACTION_DATE,String ORGAN,String PLACE,
								String VMCODE,String GOODS,String AMOUNT,String PAY_TYPE,
								String INPUT_TYPE,String PAY_STEP,String CLOSING_DATE,
								String PAY_DATE,String CANCEL_DATE) throws Exception {
	// 실행에 사용될 변수
		DBLibrary dbLib = new DBLibrary(this.logger);
		Connection conn = null;
		PreparedStatement ps = null;
...
// 정렬 값(oMode, oType)에 따라 ORDER BY 변경
	 ...
	 *
	 */
	public String column(String sType, long company, long organ, long place, String sDate, String eDate, String[] payTypes, int oMode, int oType) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sType) || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
					...
					" SELECT /*+ ORDERED USE_HASH(B C D) */"
						...
						+ " FROM ("
				        		...
				        		+ " FROM (	"					
									...
									+ " GROUP BY AA.COMPANY_SEQ, AA.ORGANIZATION_SEQ, AA.VM_PLACE_SEQ, AA.PRODUCT_CODE, NVL(E.NAME, '미등록 상품[' || AA.COL_NO || ']') " //scheo 20181214 유광권부장 요청 - 20190619 scheo 원복
...
// 정렬 값(oMode, oType)에 따라 ORDER BY 변경
	 ...
	 *
	 */
	public String salesCountList(long company, long organ, long place, String sDate, String eDate, int oMode, int oType) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
				                        ...
				                        + " FROM  TB_SALESCOUNT A "
				                            + " INNER JOIN TB_VENDING_MACHINE B"
			                                	...
			                                	+ " AND ((B.MODEL IN ('LVM-6112') AND COL_NO NOT IN (0, 12, 13)) "
			                                		...
			                                		+ " OR (B.MODEL IN ('CVK-6024') AND COL_NO NOT IN (0, 13, 14)) "
			                                		+ " OR (B.MODEL NOT IN ('LVM-6112', 'R-6107', 'CVK-6024') AND COL_NO NOT IN (0))) "
	 ...
	 *
	 */
	public String salesCountListNew(long company, long organ, long place, String sDate, String eDate, int oMode, int oType) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
				...
				"SELECT /*+ ORDERED USE_HASH(B C) */ "
				 ...
				 + " FROM (" 
				    ...
				    + " ) C"
				        ...
				        + " WHERE C.TERMINAL_ID = D.TERMINAL_ID"	//scheo 
				+ " ) A" 
	 ...
	 *
	 */
	public String salesCountRawList(long company, long organ, long place, String sDate, String eDate, int oMode, int oType) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
	 ...
	 *
	 */
	public String salesCountRawListNew(long company, long organ, long place, String sDate, String eDate, int oMode, int oType) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
	 ...
	 *
	 */
	public String salesCount(long company, long organ, long place, String sDate, String eDate, String terminal) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
	 ...
	 *
	 */
	public String salesCountNew(long company, long organ, long place, String sDate, String eDate, String terminal) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
				    ...
				    + " ) C" 
				        + " INNER JOIN TB_VENDING_MACHINE S "
				        	...
				        	+ " AND ((S.MODEL IN ('LVM-6112') AND COL_NO NOT IN (0, 12, 13)) "
				        		...
				        		+ " OR (S.MODEL IN ('CVK-6024') AND COL_NO NOT IN (0, 13, 14)) "
                        		+ " OR (S.MODEL NOT IN ('LVM-6112', 'R-6107', 'CVK-6024') AND COL_NO NOT IN (0))) " 
				+ " ) A" 
				    + " INNER JOIN TB_ORGANIZATION B ON A.ORGANIZATION_SEQ = B.SEQ" 
				    + " INNER JOIN TB_VENDING_MACHINE_PLACE C ON A.VM_SEQ = C.VM_SEQ AND C.END_DATE IS NULL" 
	 ...
	 *
	 */
	public String salesCountRaw(long company, long organ, long place, String sDate, String eDate, String terminal) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
	 ...
	 *
	 */
	public String salesCountRawNew(long company, long organ, long place, String sDate, String eDate, String terminal) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
	 ...
	 *
	 */
	public String salesCountRealTime(long company, long organ, long place, String sDate, String eDate, String terminal, int pageNo) throws Exception {
	// 필수 검색 조건이 없을 때
		if (company == 0 || StringEx.isEmpty(sDate) || StringEx.isEmpty(eDate)) {
			return null;
		}
