...
// 목록 가져오기
	...
	try {
		...
		//ps = dbLib.prepareStatement(conn, "SELECT /*+ INDEX(A PK_ORGANIZATION) */ SEQ, NAME || (CASE WHEN IS_ENABLED = 'N' THEN '(' || NVL(SUBSTR(SUM_DATE, 3, 6), '000000') || ')' ELSE '' END) AS NAME FROM TB_ORGANIZATION A WHERE " + WHERE);
		ps = dbLib.prepareStatement(conn,
				"SELECT /*+ INDEX(A) */"
						+ " SEQ, NAME || DECODE(IS_ENABLED, 'N', '(' || NVL(SUBSTR(SUM_DATE, 3, 6), '000000') || ')', '') AS NAME"
...
// 계정 가져오기
	...
	if (StringEx.isEmpty(error) && mode.equals("B") && company > 0) {
		if (cfg.getLong("user.company") > 0) { // 시스템 관리자가 아닌 경우, 내가 속한 소속만 검색
			WHERE = " COMPANY_SEQ = " + cfg.getLong("user.company");
		} else {
			WHERE = " COMPANY_SEQ = " + company;
		}
...
// 설치 위치
	...
	if (StringEx.isEmpty(error) && mode.equals("C") && company > 0) {
		...
		try {
			//20160108 JOIN 순서 변경 및 INDEX(A PK_VM) 제거, FULL 힌트 추가
