...
// 엑셀 생성
	...
	try {
		...
		sheet.mergeCells(col, row, col, row + 1);
		sheet.setColumnView(col++, 15);
		sheet.addCell(new Label(col, row, "상품명", format1));
		sheet.mergeCells(col, row, col, row + 1);
		sheet.setColumnView(col++, 50);
		...
		//'자판기 매출정보 > 매출집계 > 자판기컬럼별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 Start
		...
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		sheet.setColumnView(col++, 15);
		//'자판기 매출정보 > 매출집계 > 자판기컬럼별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24 End
		...
		for (int i = 0; i < objSales.list.size(); i++, row++) {
			...
			sheet.addCell(new Number(col++, row, c.getLong("CNT_PAYCO"), format3));		//'자판기 매출정보 > 매출집계 > 자판기컬럼별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
			sheet.addCell(new Number(col++, row, c.getLong("AMOUNT_PAYCO"), format3));	//'자판기 매출정보 > 매출집계 > 자판기컬럼별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
		...
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));	//'자판기 매출정보 > 매출집계 > 자판기컬럼별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
		sheet.addCell(formula.copyTo(col++, row));	//'자판기 매출정보 > 매출집계 > 자판기컬럼별' 엑셀 다운로드 간편결제 컬럼 추가. Chae Jongwook. 2020-04-24
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
