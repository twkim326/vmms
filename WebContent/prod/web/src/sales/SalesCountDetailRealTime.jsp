...
// 전송된 데이터
	...
	long __organ = StringEx.str2long(request.getParameter("__organ"), 0);
	long __place = StringEx.str2long(request.getParameter("__place"), 0);
	String __sDate = StringEx.getKeyword(StringEx.charset(request.getParameter("__sDate")));
	String __eDate = StringEx.getKeyword(StringEx.charset(request.getParameter("__eDate")));
	int __oMode = StringEx.str2int(request.getParameter("__oMode"), -1);
	int __oType = StringEx.str2int(request.getParameter("__oType"), -1);
	String __goUrl = StringEx.charset(request.getParameter("__goUrl"));
...
// 메서드 호출
	...
	try {
		error = objSales.salesCountRealTime(company, organSeq, depth, placeSeq, startDate, endDate, terminalId, pageNo);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<div class="infoBar">
	...
	<input type="button" class="button2" value="컬럼보기" onclick="_return();" style="right:80px;" />
	<% } %>
</div>
...
<table cellpadding="0" cellspacing="0" class="tableType01">
	<thead>
		<tr>
			...
			<th nowrap>거래번호</th>
			<th nowrap>상품</th>
			<th nowrap>컬럼</th>	<%--20200715 scheo 컬럼 추가 --%>
			<th nowrap>금액</th>
			<th nowrap>수단</th>
			<th nowrap>입력</th>
			<th nowrap>상태</th>
...
<% for (int i = 0; i < objSales.list.size(); i++) { GeneralConfig c = (GeneralConfig) objSales.list.get(i); %>
		<tr class="<%=(i % 2 == 1 ? "bgLine" : "")%>" title="단말기 ID = <%=c.get("TERMINAL_ID")%>, 거래번호 = <%=c.get("TRANSACTION_NO")%>">
			...
			<td class="center" nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("TRANSACTION_NO"), "-"))%></td>
			<td nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("PRODUCT"), "-"))%></td>
			<td class="center" nowrap><%=Html.getText(StringEx.setDefaultValue(c.get("COL_NO"), "-"))%></td> <%--20200715 scheo 컬럼 추가 --%>
...
<% if (objSales.list.size() == 0) { %>
		<tr>
			<td colspan="17" align="center">등록된 상세 매출 내역이 없습니다</td>
		</tr>
<% } %>
	</tbody>
</table>
...
<% } else { %>
		_submit($("form"), "SalesCountDetailRealTimeExcel.jsp", "_top");
<% } %>
	}
</script>