...
%><%
/**
 * /sales/SalesReportMonthlyDetail.jsp
 *
 * 자판기 매출정보 > 조건별 매출 현황 > 월별
 *
 * 작성일 - 2011/04/05, 정원광
 *
 */
...
// 전송된 데이터
	...
	if (sDate.length() > 6) sDate = sDate.substring(0, 6);
	if (eDate.length() > 6) eDate = eDate.substring(0, 6);
	if (oMode == -1) oMode = 0;
	if (oType == -1) oType = 1;
	if (payTypes != null) {
...
// URL에 더해질 파라미터
//	String addParam = Param.addParam(request, "sType:company:organ:place:sDate:eDate");
...
// 메서드 호출
	...
	try {
		out.clear();
		error = objSales.monthly(sType, company, organ,depth, place, sDate + "01", eDate + "31", payTypes, oMode, oType);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<table cellpadding="0" cellspacing="0" cellspacing="0" class="tableType01">
	<thead>
		<tr>
			<th rowspan="2" nowrap><a href="javascript:_sort(0,<%=(oMode == 0 && oType == 0 ? 1 : 0)%>);" class="<%=(oMode == 0 ? (oType == 1 ? "desc" : "asc") : "none")%>" onmouseover="this.className = '<%=(oMode == 0 ? (oType == 1 ? "asc" : "desc") : "asc")%>';" onmouseout="this.className = '<%=(oMode == 0 ? (oType == 1 ? "desc" : "asc") : "none")%>';" title="날짜별 정렬 <%=(oMode == 0 && oType == 0 ? "(↑)" : "(↓)")%>">날짜</a></th>
	...
	<%	}
		...
		// 2016012 중간에 소계 추가
		String[] groupColumns
			= (oMode == 1) ? new String[] { "ORGAN", "DATE" }
			: (oMode == 2) ? new String[] { "ORGAN", "PLACE" }
			: new String[] { "DATE", "ORGAN" };
		...
		<tr onclick="_detail('<%=c.get("YYYYMM")%>', <%=c.getLong("VM_PLACE_SEQ")%>, <%=c.getLong("AMOUNT_TOTAL")%>);" style="cursor:pointer;<%=c.getLong("AMOUNT_TOTAL") < 0L ? "background-color:#e9aaaa;" : ""%>">
...
<script type="text/javascript">
	...
	function _detail(yyyymm, place, amount) {
		$("sDate").value = yyyymm + "01";
		$("eDate").value = yyyymm + "31";
		$("place").value = place;
		if (amount < 0) {
			var sType = $("sType");
	...
	<% } else { %>
		_submit($("form"), "SalesReportMonthlyExcel.jsp", "_top");
	<% } %>
	}
</script>