...
// 메서드 호출
	...
	try {
		error = objSales.summary("02", company, organ, place, sDate, eDate, payTypes, oMode, oType);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<table id="detail" cellpadding="0" cellspacing="0" class="tableType06">
	<thead>
		<tr>
			...
			<th colspan="<%=(7 + prepayCompanyCount * 2)%>" nowrap>마감</th>
	...
	%>
		...
		<tr>
			...
			%>
		<tr onclick="_detail(<%=c.getLong("VM_PLACE_SEQ")%>, <%=c.getLong("AMOUNT_TOTAL")%>);" style="cursor:pointer;<%=payAmountTotal < 0L ? "background-color:#e9aaaa;" : (c.getLong("CNT_TOTAL") != (c.getLong("CNT_HELD") + c.getLong("CNT_DECLINED") + c.getLong("PAY_CNT_TOTAL")) ? "background-color:#ffffe9;" : "")%>">
