...
<form name="search" id="search" method="get" action="SalesCountDetail.jsp" target="detail">
...
<table cellspacing="0" class="tableType02">
	...
	<tr>
		...
		<td>
			<select id="sField" name="sField">
				...
				<option value="C.TERMINAL_ID">단말기 ID</option>
				<option value="C.PLACE_CODE">자판기코드</option>
				<option value="PLACE">설치위치</option>
				<option value="U.NAME">관리자</option>
			</select>
		<!-- <th><span>단말기 ID</span></th> -->
			<input type="text" id="sQuery" class="txtInput" name="sQuery" value="" />
		</td>
	...
	<tr>
		...
		<td>
			<select id="company" name="company"class="<%=(cfg.getLong("user.company") > 0 ? "disabled" : "")%>" onchange="Company.organ(this.options[selectedIndex].value.evalJSON(), 0, 0, 'C', null, 'Y');"<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%>>
	...
	<tr>
		...
		<td>
			<input type="text" name="sDate" id="sDate" value="<%=sDate%>"class="txtInput" style="width:80px;" maxlength="8" onkeyup="if (this.value != '' && !/^[0-9]+$/.test(this.value)) { window.alert('숫자만 입력이 가능합니다.'); this.value = ''; }" />
			...
			~
			<input type="text" name="eDate" id="eDate" value="<%=eDate%>"class="txtInput" style="width:80px;" maxlength="8" onkeyup="if (this.value != '' && !/^[0-9]+$/.test(this.value)) { window.alert('숫자만 입력이 가능합니다.'); this.value = ''; }" />
			<img src="<%=cfg.get("imgDir")%>/web/icon_calendar.gif" style="cursor:pointer;" alt="날짜 선택" onclick="(function () {
					var e = event ? event : window.event;
