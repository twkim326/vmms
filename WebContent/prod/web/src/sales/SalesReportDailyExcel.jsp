...
%><%
/**
 * /sales/SalesReportDailyExcel.jsp
 *
 * 자판기 매출정보 > 조건별 매출집계 > 일별 > 엑셀
 *
 * 작성일 - 2011/04/05, 정원광
 *
 */
...
// 메서드 호출
	...
	try {
		error = objSales.daily(sType, company, organ, depth, place, sDate, eDate, payTypes, oMode, oType);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
// 엑셀 생성
	...
	try {
		...
		sheet.addCell(new Label(col, row + 0, "설치위치", format1));
		sheet.mergeCells(col, row, col, row + 1);
		sheet.setColumnView(col++, 50);
		sheet.addCell(new Label(col, row + 0, "자판기코드", format1));
		sheet.mergeCells(col, row, col, row + 1);
		sheet.setColumnView(col++, 10);

		sheet.addCell(new Label(col, row + 0, "현금", format1));
		...
		sheet.addCell(new Label(col, row + 0, "신용", format1));
		sheet.mergeCells(col, row, col + 1, row);
		sheet.addCell(new Label(col, row + 1, "건수", format1));
		sheet.setColumnView(col++, 10);
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		...
		//2019.02.25 scheo 추가
		sheet.addCell(new Label(col, row + 0, "간편결제", format1)); 
		sheet.mergeCells(col, row, col + 1, row);
		sheet.addCell(new Label(col, row + 1, "건수", format1));
		sheet.setColumnView(col++, 10);
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		...
		sheet.addCell(new Label(col, row + 0, "선불", format1));
		sheet.mergeCells(col, row, col + 1, row);
		sheet.addCell(new Label(col, row + 1, "건수", format1));
		sheet.setColumnView(col++, 10);
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		...
		sheet.addCell(new Label(col, row + 0, "정보수집", format1));
		sheet.mergeCells(col, row, col + 1, row);
		sheet.addCell(new Label(col, row + 1, "건수", format1));
		sheet.setColumnView(col++, 10);
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		...
		sheet.addCell(new Label(col, row + 0, "합계", format1));
		sheet.mergeCells(col, row, col + 1, row);
		sheet.addCell(new Label(col, row + 1, "건수", format1));
		sheet.setColumnView(col++, 10);
		sheet.addCell(new Label(col, row + 1, "금액", format1));
		...
		for (int i = 0; i < objSales.list.size(); i++, row++) {
			...
			sheet.addCell(new Number(col++, row, c.getLong("CNT_PAYCO"), format3));	//2019.02.25 scheo 추가
			sheet.addCell(new Number(col++, row, c.getLong("AMOUNT_PAYCO"), format3)); //2019.02.25 scheo 추가
		...
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row)); //2019.02.25 scheo 추가
		sheet.addCell(formula.copyTo(col++, row)); //2019.02.25 scheo 추가
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
		sheet.addCell(formula.copyTo(col++, row));
...
// 파일명
	String excelName = StringEx.encode("일별매출집계");
