...
// 전송된 데이터
	...
	long company = StringEx.str2long(request.getParameter("company"), 0) > 0? StringEx.str2long(request.getParameter("company"), 0) : cfg.getLong("user.company") ;
	long organ =   StringEx.str2long(request.getParameter("organ"), 0) > 0 ?  StringEx.str2long(request.getParameter("organ"), 0) : cfg.getLong("user.organ") ;
	int depth = cfg.getInt("user.organ.depth");
// 인스턴스 생성
...
<iframe id="detail" name="detail" src="SalesRealTimeDetail.jsp" scrolling="no" frameBorder="0" class="mt23" style="width:100%; height:207px; border-style:none;"></iframe>
...
<% 	if (depth == 0 ) { %>	
		Company.company(<%=(cfg.getLong("user.company") > 0 ? cfg.getLong("user.company") : company)%>, <%=(organ > 0 ? organ : cfg.getLong("user.organ"))%>, 0, 'C', null, 'Y');
<% 	} else if (depth == 1 ) { %>		
		CompanyEX2.company(<%=(cfg.getLong("user.company") > 0 ? cfg.getLong("user.company") : company)%>, <%=(organ > 0 ? organ : cfg.getLong("user.organ"))%>, 0, 'C', null, 'Y');
<% 	} else if (depth == -1 ) { %>	
		Company.company(<%=(cfg.getLong("user.company") > 0 ? cfg.getLong("user.company") : company)%>, <%=(organ > 0 ? organ : cfg.getLong("user.organ"))%>, 0, 'C', null, 'Y');
<% 	}  %>	
