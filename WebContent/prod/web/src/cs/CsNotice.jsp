...
// 메서드 호출
	...
	try {
		error = objNotice.getList(cate, pageNo, sField, sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
...
<table cellspacing="0" class="tableType02">
	...
	<tr>
		...
		<td rowspan="2" class="center">
			<input type="submit" value="" class="btnSearch<%=(objNotice.cate.size() == 0 ? "S" : "")%>" />
		</td>
	</tr>
<% if (objNotice.cate.size() > 0) { %>
	<tr>
		<th><span>카테고리</span></th>
		<td>
			<select id="cate" name="cate">
				<option value="">- 선택하세요</option>
	<% for (int i = 0; i < objNotice.cate.size(); i++) { GeneralConfig c = (GeneralConfig) objNotice.cate.get(i); %>
...
<div class="infoBar mt23">
	검색 결과 : <strong><%=StringEx.comma(objNotice.records)%></strong>건
<% if (cfg.isAuth("I")) { %>
	<input type="button" class="button1" value="등록" onclick="location.href = 'CsNoticeRegist.jsp?1=1<%=addParam%>';" />
<% } %>
</div>
<table cellpadding="0" cellspacing="0" class="tableType01">
	...
	<tbody>
<% for (int i = 0; i < objNotice.list.size(); i++) { GeneralConfig c = (GeneralConfig) objNotice.list.get(i); %>
		<tr class="<%=(i % 2 == 1 ? "bgLine" : "")%>" onclick="location.href = 'CsNoticeDetail.jsp?seq=<%=c.getLong("SEQ") + addParam%>';" style="cursor:pointer;">
...
<!-- 페이징 -->
<div class="paging"><%=Pager.getList(request, pageNo, cfg.getInt("limit.page"), objNotice.pages, "", "page", "", buttons)%></div>
<!-- # 페이징 -->

<!--공지사항 첨부파일-->
<div style="margin-top:20px;">
	<div align="center">		
		<a download href="https://vmms.ubcn.co.kr/web/src/Product_registration.pdf" data-filesize="1.13" class="download-button"style="padding: 10px 10px 10px 54px; margin-top: 0px;">상품등록 매뉴얼</a>	
		<a download="" href="https://vmms.ubcn.co.kr/download/Mch_change_form.zip" data-filesize="0.363" class="download-button"style="padding: 10px 10px 10px 54px; margin-top: 0px;">명의이전 신청양식</a>
		...
		<a download="" href="https://vmms.ubcn.co.kr/download/Terminal_ID_Change_Guide.pdf" data-filesize="0.213" class="download-button"style="padding: 10px 10px 10px 54px; margin-top: 0px;">단말기 내 TID 변경 방법.pdf</a>	
	</div>
</div>
<!-- #공지사항 첨부파일-->
