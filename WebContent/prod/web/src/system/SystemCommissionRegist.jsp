...
%><%
/**
 * /system/SystemCommissionRegist.jsp
 *
 * 시스템 > 수수료 > 등록
 *
 * 작성일 - 2011/06/11, 정원광
...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		try {
			error = objCommission.regist(company, organ, card, commission, sDate, eDate, memo);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<div id="window">
	...
	<table cellspacing="0" class="tableType03 tableType05" id="regist">
		...
		<tr>
			<th><span>소속</span></th>
			<td>
				<select id="company" class="checkForm<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%>" onchange="Company.organ(this.options[selectedIndex].value.evalJSON(), 0, 0);"<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%> option='{"isMust" : true, "message" : "소속을 선택하세요."}'>
		...
		<tr>
			<th><span>매입사</span></th>
			<td>
				<select id="card" class="checkForm" option='{"isMust" : true, "message" : "매입사를 선택하세요."}'>
					<option value="">- 선택하세요</option>
			<% for (int i = 0; i < objCommission.card.size(); i++) { GeneralConfig c = (GeneralConfig) objCommission.card.get(i); %>
					<option value="<%=c.get("CODE")%>"><%=c.get("NAME")%></option>
			<% } %>
				</select>
			</td>
		</tr>
		...
		<tr>
			...
			<td class="last"><input type="text" id="memo" class="txtInput" maxlength="200" style="width:370px" /></td>
		</tr>
	</table>
	...
	function _save() {
		...
		for (var i = $('company').value.evalJSON().depth; i >= 0; i--) {
			...
			if (o && o.selectedIndex > 0) {
				organ = o.options[o.selectedIndex].value;
				break;
			}
		}
		...
		else if (parseInt(sDate) < parseInt(Common.ymd())) {
		...
		new Ajax.Request(location.pathname, {
			asynchronous : true,
			parameters : {
				company : $('company').value.evalJSON().seq
			,	organ : organ
			,	card : $('card').value
...
//	Event.observe(window, 'load', function (event) {
		...
		Company.company(<%=cfg.getLong("user.company")%>, <%=cfg.getLong("user.organ")%>);
//	}.bind(this));
</script>