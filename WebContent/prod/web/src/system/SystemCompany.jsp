...
// 메서드 호출
	...
	try {
		error = objCompany.getList(pageNo, sField, sQuery);
	} catch (Exception e) {
		error = e.getMessage();
	}
	...
	<tr>
		...
		<td>
			<select name="sField" id="sField">
				...
				<option value="A.NAME"<%=sField.equals("A.NAME") ? " selected" : ""%>>소속</option>
...
<div class="infoBar mt23">
	검색 결과 : <strong><%=StringEx.comma(objCompany.records)%></strong>건
<% if (cfg.isAuth("I")) { %>
	<input type="button" class="button2" value="소속등록" onclick="_regist();" />
<% } %>
</div>
<table cellpadding="0" cellspacing="0" cellspacing="0" class="tableType01">
	<colgroup>
		...
		<col width="80" />
		<col width="80" />
		<col width="80" /><%--2020-12-03 김태우 추가--%>
		<col width="80" />
		<col width="80" />
		<col width="80" />
	...
	<thead>
		<tr>
			...
			<th nowrap>판매계수</th>
			<th nowrap>SMS발송</th>
			<th nowrap>알림톡발송</th> <%--2020-12-03 김태우 추가--%>
			<th nowrap>등록일</th>
			<th nowrap>수정일</th>
	<% if (cfg.isAuth("U") || cfg.isAuth("D")) { %>
			<th nowrap>관리</th>
	...
	<tbody>
<% for (int i = 0; i < objCompany.list.size(); i++) { GeneralConfig c = (GeneralConfig) objCompany.list.get(i); %>
		<tr class="<%=(i % 2 == 1 ? "bgLine" : "")%>">
			...
			<td class="center"><%=c.get("IS_USED_CLOSING").equals("Y")?"처리함":"처리안함"%></td>
			<td class="center"><%=c.get("IS_VIEW_CLOSING").equals("Y")?"출력함":"출력안함"%></td>
			<td class="center"><%=c.get("IS_RAW_SALESCOUNT").equals("Y")?"보정안함":"보정함"%></td>
			<td class="center"><%=c.get("IS_SMS_ENABLED").equals("Y")?"발송함":"발송안함"%></td>
			<td class="center"><%=c.get("IS_KAKAO_ENABLED").equals("Y")?"발송함":"발송안함"%></td> <%--2020-12-03 김태우 추가--%>
...
<!-- 페이징 -->
<div class="paging"><%=Pager.getList(request, pageNo, cfg.getInt("limit.page"), objCompany.pages, "", "page", "", buttons)%></div>
<!-- # 페이징 -->
...
<script type="text/javascript">
	...
	function _regist() {
		new IFrame(510, 180, 'SystemCompanyRegist.jsp').open();
	}

	function _modify(n) {
		new IFrame(510, 180, 'SystemCompanyModify.jsp?seq=' + n).open();
	}
	...
	function _delete(n) {
		...
		new Ajax.Request('SystemCompanyDelete.jsp', {
			asynchronous : true,
			parameters : {seq : n},
