...
%><%
/**
 * /system/SystemCompanyModify.jsp
 *
 * 시스템 > 소속 > 수정
 *
 * 작성일 - 2011/03/25, 정원광
...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		try {
			//error = objCompany.regist(seq, name, memo, isUsedClosing, isViewClosing, StringEx.split(StringEx.arrange(group, ",", ","), ","));
			//error = objCompany.regist(seq, name, memo, isUsedClosing, isViewClosing, isRawSalescount, isSMSEnabled, StringEx.split(StringEx.arrange(group, ",", ","), ","));
			/*2020-12-03 김태우 추가*/
			error = objCompany.regist(seq, name, memo, isUsedClosing, isViewClosing, isRawSalescount, isSMSEnabled, isKAKAOEnabled, kAKAOMaxCount,StringEx.split(StringEx.arrange(group, ",", ","), ","));
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<div id="window">
	...
	<table cellspacing="0" class="tableType03 tableType05" id="regist">
		...
		<tr>
			<th><span>소속</span></th>
			<td><input type="text" id="name" class="checkForm txtInput" value="<%=Html.getText(objCompany.data.get("NAME"))%>" maxlength="20" option='{"isMust" : true, "message" : "소속을 입력하세요."}' /></td>
		</tr>
		<tr>
			<th><span>설명</span></th>
			<td><input type="text" id="memo" class="checkForm txtInput" value="<%=Html.getText(objCompany.data.get("MEMO"))%>" maxlength="200" style="width:370px" /></td>
		</tr>
		<tr>
			<th><span>마감 처리</span></th>
			<td>
				<select id="isUsedClosing">
					<option value="N"<%=(objCompany.data.get("IS_USED_CLOSING").equals("Y") ? "" : " selected")%>>처리안함</option>
					<option value="Y"<%=(objCompany.data.get("IS_USED_CLOSING").equals("Y") ? " selected" : "")%>>처리함</option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select id="isViewClosing">
					<option value="N"<%=(objCompany.data.get("IS_VIEW_CLOSING").equals("Y") ? "" : " selected")%>>출력안함</option>
					<option value="Y"<%=(objCompany.data.get("IS_VIEW_CLOSING").equals("Y") ? " selected" : "")%>>출력함</option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select id="isRawSalescount">
					<option value="N"<%=(objCompany.data.get("IS_RAW_SALESCOUNT").equals("Y") ? "" : " selected")%>>보정함</option>
					<option value="Y"<%=(objCompany.data.get("IS_RAW_SALESCOUNT").equals("Y") ? " selected" : "")%>>보정안함</option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select id="isSMSEnabled">
					<option value="N"<%=(objCompany.data.get("IS_SMS_ENABLED").equals("Y") ? "" : " selected")%>>발송안함</option>
					<option value="Y"<%=(objCompany.data.get("IS_SMS_ENABLED").equals("Y") ? " selected" : "")%>>발송함</option>
				</select>
			</td>
		</tr>
		<%--2020-12-03 김태우 추가 / 알림톡 발송--%>
		<tr>
			<th><span>알림톡발송</span></th>
			<td>
				<select id="isKAKAOEnabled" onchange="_change()">
					<option value="N"<%=(objCompany.data.get("IS_KAKAO_ENABLED").equals("Y") ? "" : " selected")%>>발송안함</option>
					<option value="Y"<%=(objCompany.data.get("IS_KAKAO_ENABLED").equals("Y") ? " selected" : "")%>>발송함</option>
				</select>
				<%--2020-12-09 김태우 추가 / 알림톡 최대개수--%>
				<span id="showKakaoCount" style="display: <%=(objCompany.data.get("IS_KAKAO_ENABLED").equals("Y") ? " block" : "none")%>;">
					※월별 최대 발송개수 :
					<input id="kAKAOMaxCount" type="number" value="<%=objCompany.data.get("KAKAO_MAX_COUNT")%>"  style="width: 40px;"/>
				</span>
			</td>
		</tr>
		<tr>
