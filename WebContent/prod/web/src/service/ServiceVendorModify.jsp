...
%><%
/**
 * /service/ServiceVendorModify.jsp
 *
 * 서비스 > 상품 공급자 관리 > 수정
 *
 * 작성일 - 2017/04/28, 황재원
...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		try {
			error = objVendor.regist(seq, name, memo);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<div id="window">
	...
	<table cellspacing="0" class="tableType03 tableType05" id="regist">
		...
		<tr>
			...
			<td><input type="text" id="name" class="checkForm txtInput" value="<%=Html.getText(objVendor.data.get("NAME"))%>" maxlength="20" option='{"isMust" : true, "message" : "소속을 입력하세요."}' /></td>
		</tr>
		<tr>
			<th><span>설명</span></th>
			<td><input type="text" id="memo" class="checkForm txtInput" value="<%=Html.getText(objVendor.data.get("MEMO"))%>" maxlength="200" style="width:370px" /></td>
		</tr>
	</table>
