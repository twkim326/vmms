...
%><%
/**
 * /service/SystemVMMasterRegist.jsp
 *
 * 서비스 > 자판기 기초정보 > 등록
 *
 * 작성일 - 2011/04/02, 정원광
...
// 메서드 호출
	...
	if (request.getMethod().equals("POST")) {
		...
		if (company == 0 || (!isAuto && StringEx.isEmpty(code)) || StringEx.isEmpty(place)) {
			out.print("{\"code\" : \"FAIL\", \"message\" : \"" + URLEncoder.encode("필수 입력값이 존재하지 않습니다.", "UTF-8") + "\"}");
			return;
		}

		try {
			error = objVM.master(company, code, region, place, model, isAuto);
		} catch (Exception e) {
			error = e.getMessage();
		}
...
<div id="window">
	...
	<table cellspacing="0" class="tableType03 tableType05" id="regist">
		...
		<tr>
			<th><span>소속</span></th>
			<td>
				<select id="company" class="checkForm<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%> js-example-basic-single js-example-responsive" style="width: 50%" option='{"isMust" : true, "message" : "소속을 선택하세요."}'<%=(cfg.getLong("user.company") > 0 ? " disabled" : "")%>>
		...
		<tr>
			<th><span>코드</span></th>
			<td><input type="text" id="code" class="checkForm txtInput" maxlength="20" option='{"isMust" : true, "message" : "코드를 입력하세요."}' /> <!-- scheo 20180720 조성범 주임 요청 option='{"isMust" : true, "varType" : "ALNUM", "message" : "코드를 입력하세요."}' /> --> <input type="checkbox" id="isAuto" class="checkbox" onclick="if (this.checked) { $('code').value = ''; $('code').disabled = true; $('code').addClassName('disabled'); } else { $('code').disabled = false; $('code').removeClassName('disabled'); }" /><label for="isAuto">자동생성</label></td>
		</tr>
		<tr>
			<th><span>설치지역</span></th>
			<td><input type="text" id="region" class="checkForm txtInput" maxlength="100" style="width:370px" /></td>
		</tr>
		<tr>
			<th><span>설치위치</span></th>
			<td><input type="text" id="place" class="checkForm txtInput" maxlength="200" style="width:370px" option='{"isMust" : true, "message" : "설치위치를 입력하세요."}' /></td>
		</tr>
		<tr>
			<th class="last"><span>모델</span></th>
			<td class="last"><input type="text" id="model" class="checkForm txtInput" maxlength="200" /></td>
		</tr>
	</table>
